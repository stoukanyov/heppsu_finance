...
> ## ⚠️ ОСТАРЯЛ — не следвай този файл
>
> Описва инфраструктура, която **не съществува**: Caddy за TLS, приложението в
> `/opt/ai-finance-os`, `git pull` върху сървъра, единична машина. Реалността е
> две машини (Phobos и Deimos), nginx + certbot в контейнери, деплой само от git
> таг през `infra/deploy.sh`, код в `/srv/aifos/<env>/release/`.
>
> **Актуалният документ е [`infra/README.md`](../infra/README.md).** Този файл
> се пази само защото раздели 4 и 9 (fail-fast за `SECRET_KEY`, ограничаването на
> опитите за вход, чеклистът по сигурност) още описват вярно поведението на
> приложението. Всичко останало е историческо.
