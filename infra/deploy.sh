...
#   3. Прави бекъп на базата ПРЕДИ миграциите и спира, ако бекъпът се провали.
...
# ───────────────────── 3. Бекъп ПРЕДИ каквато и да е миграция ─────────────────
log "Бекъп на базата преди миграциите"
...
if [ -d release ] && dc ps --status running --services 2>/dev/null | grep -qx db; then
    ...
    gzip -t "$OUT" || { echo "ГРЕШКА: повреден бекъп — СПИРАМ" >&2; exit 1; }
    echo "  бекъп: $OUT ($(numfmt --to=iec "$SIZE" 2>/dev/null || echo "$SIZE B"))"
else
    echo "  средата е нова — няма какво да се бекъпва"
fi
REMOTE
