...
# nginx се пресъздава ПРИНУДИТЕЛНО при всяко разгръщане.
#
# Конфигурацията му се монтира от `release/infra/nginx/conf.d`, а редът по-горе
# сменя самата `release/` с преименуване. Bind mount-ът обаче е вързан за inode-а,
# разрешен при пускането на контейнера — не за пътя. След смяната контейнерът
# гледа в старата директория, а `rm -rf release.old` при следващото разгръщане я
# изтрива. Тогава `/etc/nginx/conf.d` вътре става ПРАЗНА.
#
# Това не се вижда: nginx държи конфигурацията в паметта си и продължава да
# обслужва нормално. Счупва се чак при следващото презареждане — а него го прави
# certbot след подновяване на сертификата, в 01:30 през нощта. Тогава nginx се
# връща без нито един `server` блок, спира да слуша изобщо, и целият сайт пада.
# `docker ps` показва „Up", `nginx -t` казва „syntax is ok", защото празната
# конфигурация е валидна конфигурация.
#
# Точно това се случи на 02.08.2026: production беше паднал седем часа, преди
# деплой да го открие. `--force-recreate` струва секунда прекъсване и връзва
# монтирането за текущата `release/`.
