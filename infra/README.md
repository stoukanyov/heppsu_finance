...
Правилата на Ив от 23.08.2026: **всеки деплой минава първо през тест**, а
**деплой на production се случва само след негово одобрение**. Тестовата среда
тук е `preprod`.

```bash
./infra/deploy.sh preprod v1.2.0          # тест
./infra/deploy.sh prod    v1.2.0 <sha>    # production — иска одобрение
```

**1. Минало ли е през preprod.** Успешен деплой на `preprod` слага етикет
`tested/<sha>` в origin — след като health gate е минал, не след като docker е
казал „up". Production отказва комит, който не се съдържа в нито един такъв
етикет.

Проверява се *ancestry*, а не съвпадение: таг и клон са различни обекти дори
при еднакво съдържание. Ако целта е предшественик на нещо тествано, значи
всичко в нея вече е било на preprod.

Записът е етикет в git, а не файл на машина: преживява пресъздаването на
средата, вижда се отвсякъде, и не спира production деплоя, когато Deimos е
недостъпен.

**2. Одобрението.** Третият аргумент е самият sha, не флаг „да" — „да" одобрява
каквото се случи да е на рефа в момента, а sha одобрява точно това, което е
било показано. Придвижи ли се рефът между показването и пускането, скриптът
отказва. Без одобрение печата кои комити ще влязат и спира.

### Трети предпазител: Definition of Done

Списъкът е в `infra/dod.yml`, не в скрипта — расте, без да се пипа логиката, и
се вижда в диф какво е било изисквано кога.

| | как се проверява |
|---|---|
| функция за докладване | маршрут на **preprod** |
| security scan | етикет `security/<sha>` от Security Officer |
| правно ревю | етикет `legal/<sha>` от Legal Officer |

**Това приложение НЕ отговаря на първото изискване.** Проверено на 24.08.2026 по
`openapi.json` на preprod: има счетоводни отчети и `/api/v1/mobile/crash-reports`
(автоматични сривове от клиента) — нито едното не е докладване от човек.

Тоест **production деплой ще отказва, докато такава функция не се появи.** Това
е целта, не страничен ефект: счетоводна система, в която потребителят няма как да
съобщи проблем, не бива да приема нови версии.

Заданието за двете сесии е в `docs/DOD-ETIKETI.md`.

**Пропускане може да нареди САМО Ив** — `AIFOS_DOD_PROPUSNI="<sha>:<причина>"`.
Никоя съседна сесия не може да отмени проверка; такава заявка се отказва и се
показва на него.

> **Границата, честно.** Скрипт не може да различи Ив от сесия, написала същия
...
`demo` минава без двата — тя е витрина, не production.

Тестовете са в `apps/api/tests/test_infra_deploy_gates.py`; вдигат истинско git
хранилище и пускат скрипта наистина.

### Клеймо за успех

След последната си стъпка скриптът пише unix време в
`/srv/aifos/prod/backups/LAST_SUCCESS`. Това е единственото, което провален бекъп
не може да напише: файлове в директорията има и когато дъмпът е празен, ред в
crontab има и когато скриптът гърми всяка нощ, а логът го чете само този, който
вече е заподозрял нещо.

Форматът (`date -u +%s`) и мястото са същите като при Groomelle, My Beagle и
Control Center — нарочно, за да може една проверка да гледа и четирите. Control
Center мери възрастта; над 26 часа значи пропусната нощ.

#### Скриптът умираше всяка нощ (открито 22.08.2026)

Ротацията беше `ls -1t <шаблон> | tail | xargs -r rm -f`. При несъвпаднал шаблон
`ls` излиза с грешка, `set -o pipefail` я вдига до целия конвейер, а `set -e`
убива скрипта. `documents/` е празна, докато `storage/` е празна — тоест третият
ред гърмеше **всяка нощ**, след записания дъмп, но преди края.

В целия `backup.log` от 26.07 до 22.08.2026 нямаше **нито едно** „готово". Никой
не забеляза, защото копието си беше на място, а изходният код на cron не се чете
от никого. Дневните и седмичните копия се въртяха (те са преди счупения ред);
всичко след него не се е изпълнявало нито веднъж.

Ротацията вече минава през `find`, който върху нищо излиза с 0. Празна
директория е обикновен случай, а не край на света.
